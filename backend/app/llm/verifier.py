...
REMEDY_PROMPT = """你是学习补强任务设计者。基于原任务和错题反馈，生成 1-2 个补强任务帮助学生巩固薄弱点。
输出 JSON 格式：
{"tasks": [{"title": "补强任务标题", "description": "具体可执行的任务内容", "type": "learn", "effort_hours": 0.5}]}
规则：effort_hours 固定 0.5，type 固定 learn，描述用中文。只输出 JSON。"""


class RemedyTask(BaseModel):
    title: str
    description: str = ""
    type: str = "learn"
...
class RemedyResult(BaseModel):
    tasks: list[RemedyTask]


def generate_remedy_tasks(
    task_title: str, task_description: str, feedback: str, client=None
) -> RemedyResult:
    payload = {"原任务": task_title, "内容": task_description or "无", "错题反馈": feedback}
    return _parse(
        client, REMEDY_PROMPT,
        json.dumps(payload, ensure_ascii=False),
        RemedyResult,
    )
